...
/*
 ...
 @RequestMapping
 	...
 	호출하므로 메서드명은 큰 의미가 없다. 개발자가 구분하기 좋은
 	정도의 이름으로 설정하면 된다.
 */
@Controller 
public class FormController {
	...
	/*
	 폼값받기1] 파라미터로 전달된 값을 HttpServletRequest 내장 객체를 
	...
	/*
	 폼값받기2] @Requestparam어노테이션으로 폼값받기
	...
	/*
	 ...
	 조건1 : 쿼리 스트링으로 전달되는 파라미터의 갯수와 폼값을 저장할 객체(
	 DTO혹은 VO)의 멤버변수의 갯수가 동일한 것을 권장한다.
	 조건2 : 커맨드 객체를 사용할때는 클래스명 앞글자를 소문자로 바꾼 형태의 매개변수를 
	 사용해야 한다. 
	 EX) MemberDTO -> memberDTO
	 조건3 : 커맨드객체를 만들때 파라미터의 갯수는 달라도 상관없으나
	 파라미터명과 멤버변수명은 동일해야 되고 이에 해당하는 getter/setter가 반드시
	 생성되어야 한다.
	 */
	...
	/*
	 ...
	 웹 브라우저는 요청명을 경로로 인식하므로 리소스를 사용할때
